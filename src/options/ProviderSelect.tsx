...
const ConfigPanel: FC<ConfigProps> = ({ config, models }) => {
  ...
  const { bindings: apiKeyBindings } = useInput(config.configs[ProviderType.GPT4]?.apiKey ?? '')
  const [model, setModel] = useState(config.configs[ProviderType.GPT4]?.model ?? models[0])
  const { setToast } = useToasts()

  console.debug(tab)

  const save = useCallback(async () => {
    if (tab === ProviderType.GPT4) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <Tabs value={tab} onChange={(v) => setTab(v as ProviderType)}>
        ...
        <Tabs.Item label="BARD webapp" value={ProviderType.Bard}>
          Use bard for searching
        </Tabs.Item>
        <Tabs.Item label="A-I-D" value={ProviderType.ChatGPTPrompt}>
          Use A-I-D for searching
        </Tabs.Item>
        <Tabs.Item label="OpenAI API" value={ProviderType.GPT4}>
